        ...
        <div class="content">
			<div class="content table-responsive table-full-width">
			    <table class="table table-striped">
			        ...
			        <tbody>
			        	<c:forEach items="${sortPB.list}" var="sort" >
				        	<tr id="tp${sort.id}">
				            	<td>${sort.id}</td>
				            	<td>${sort.name}</td>
				            	<td class="stats">
			    ...
			    <div style="text-align: center; font-size: 16px;">
						<c:if test="${sortPB.pageNum>1 }">

							<a style="margin-right: 10px;" href="sortList.adminAction?page=1">首页</a>
							<a style="margin-right: 10px;"
								href="sortList.adminAction?page=${sortPB.pageNum-1}">上一页</a>
						</c:if>
						
						<!-- 参考百度 左五右四  ，左边显示5个数，右边显示4个数-->
						<c:forEach begin="${sortPB.pageNum-5>0?sortPB.pageNum-5:1}"
							end="${sortPB.pageNum+4<sortPB.totalPage?sortPB.pageNum+4:sortPB.totalPage}"
							var="i">

							<!-- 当为当前页时不可以点击 -->
							<c:if test="${sortPB.pageNum==i }">
							...
							<c:if test="${sortPB.pageNum!=i }">
								<span >
									...
									href="sortList.adminAction?page=${i}">${i}</a>
								</span>
							</c:if>

						</c:forEach>

						<c:if test="${sortPB.pageNum<sortPB.totalPage && sortPB!=null}">

							<a style="margin-right: 10px;"
								href="sortList.adminAction?page=${sortPB.pageNum+1}">下一页</a>
							<a style="margin-right: 10px;"
								href="sortList.adminAction?page=${sortPB.totalPage}">尾页</a>
						</c:if>
					</div>
