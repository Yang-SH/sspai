				...
				if($("[name=user]").val()!=null && $("[name=user]").val()!=""){
					$.ajax({
						...
						success:function(num){
						 	//alert(num);
							if(num>0){
								//alert("queryAllComment.articleAction?articleId="+$("[name=articleId]").val());
					 			$.ajax({
					 				type: "post",
					 				...
					 				success :function(comments) {
										...
										$(".comment-list").html("");
										//alert();
										$("#ping").html("评论("+comments.length+")");
										for(var i=0;i<comments.length;i++){
											var comment=comments[i];
											var testDate = new Date();
											//testDate.setTime(comment.create_time);//时间的毫秒数进行转化，例如150237984000毫秒
											var time = testDate.Format("yyyy年MM月dd日 hh:mm:ss");
											var $comment=$("<li class='comment-item'><a class='avatar' href='javascript:;'><img alt='VkinG' src='/sspai"+comment.user.head_path+"' style='width:40px;heigth:40px;'></a><section><h5>"+comment.user.nickname+"</h5><div class='body'>"+comment.content+"</div><div class='ext'><time>"+time+"</time></div></section></li>");
											//alert($comment);
				...
				if($("[name=user]").val()!=null && $("[name=user]").val()!=""){
					$.ajax({
						...
						success:function(result){
							if(result==1){
								alert("收藏成功！");
								$("[class=btn-collection]").addClass("color");
							}else{
								alert("已取消收藏！");
								$("[class=btn-collection]").removeClass("color");
							}
						}		
					});
				}else{
...
<body style="background-color:#FAFBFC">
	...
	<div class="main container">
		<div class="article-detail">
			<article>
				<img src="${pageContext.request.contextPath}${article.pic_path}" class="banner">
				<h1 class="title">${article.article_name}</h1>
				<div class="meta">
					<div class="user-card size60">
						<a href="findUser.userAction?userid=${article.user.id}" class="avatar">
							<img src="${pageContext.request.contextPath}${article.user.head_path}" class="img-circle img-responsive" style="width: 30px;height: 30px;">
						</a>
						<h4><a href="findUser.userAction?userid=${article.user.id}">${article.user.nickname }</a></h4>
				...
				<div id="article-content" class="article-content">
                	...
                	<section class="action-region">
	                	...
	                	<span style="text-decoration: none;font-size:40px;" class="btn-collection">
	                		<i class="ti-tag"></i>
	                	</span>
                	...
                	<section class="affiliate-info">
                		<div class="item">
                			<a href="findUser.userAction?userid=${article.user.id}" class="avatar image">
                				<img class="img-circle img-responsive" src="${pageContext.request.contextPath}${article.user.head_path}" style="width: 80px;height: 80px;">
                			</a>
                			<div class="body">
                				<h4></h4>
                				<h4><a href="findUser.userAction?userid=${article.user.id}">${article.user.nickname}</a></h4>
                	...
                	<div id="comment-box" class="comment-box">
                		<%-- <form class="comment-form" action="comment.articleAction?articleId=${article.id }&userId=${article.user.id }" method="post"> --%>
                			<div class="form-group">
                				<c:if test="${loginUser!=null }">
	                				<a class="avatar" style="float: left; margin-right: 20px;" id="pl" href="findUser.userAction?userid=${loginUser.id}">
                		...
                		<!-- </form> -->
                		<br><br><br>
                		<h3 id="ping">评论（${article.comment_num }）</h3>
                		<div>
                			<ul class="comment-list">
		                		<c:forEach var="comment" items="${requestScope.listComment }">
	                				<li class="comment-item">
	                					<a class="avatar" href="findUser.userAction?userid=${comment.user.id}">
	                						<img alt="VkinG" src="${pageContext.request.contextPath}${comment.user.head_path}" lazy="loaded" style="width: 40px; height: 40px;">
	                					</a>
	                					<section>
