...
function sendRequest(){
	$.ajax({
		type:'post',
		url: 'updateTopic.adminAction', 
		data: 'id='+$('[name=id]').val()+'&name='+$('[name=name]').val()+'&is_suggest='+$('[name=is_suggest]').val()+'&stats='+$('[name=stats]').val(),
		dataType: 'json',
		success: function(data) {
			console.log(data);
			if(data.type=='fail'){//失败
				showMsg('专题数据更新失败！','danger');
			}else{
				showMsg('更新成功！','info');
			}
		}
