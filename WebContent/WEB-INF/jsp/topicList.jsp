...
<body style="background-color: #FAFBFC">
			...
			<c:when test="${loginUser!=null }">
				...
				<div class="collapse navbar-collapse" id="example-navbar-collapse navbar-toggleable-xs">
					<ul style="margin-top: 4px;" class="nav navbar-nav pull-right">
						...
						<li class="search_inp">
							<form id="s" action="query.articleAction" method="post">
	...
	<!-- 主题 -->
	<div class="main">
		<div class="topic-plaza view container">
			<header style="padding-left: 25px;">
				<div class="inner">
					<div class="recommend-topic">
						<c:forEach items="${suggestTopics}" var="sTopic" >
							<a href="findByTopic.articleAction?topicId=${sTopic.id}" class="topic-card lager">
								<img class="banner" src="${pageContext.request.contextPath}${sTopic.pic_path}"" lazy="loaded">
								<div class="mask">
									<h4>${sTopic.name}</h4>
									<footer>
										<em>${sTopic.article_num}</em>
			...
			<div class="topic-list topic-list_bottom">
				<c:forEach items="${allTopics}" var="aTopic" >
					<a href="findByTopic.articleAction?topicId=${aTopic.id}" class="topic-card">
						<img class="banner" src="${pageContext.request.contextPath}${aTopic.pic_path}">
						<div class="mask">
							<h4>${aTopic.name}</h4>
							<footer>
								<em>${aTopic.article_num}</em>
								<button class="btn">收藏</button>
							</footer>
						</div>
					</a>
